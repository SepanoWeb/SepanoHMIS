...
public class Factor {
    ...
    public static String _comment2 = "account_factor_comment2";
    public static String _comment3 = "account_factor_comment3";
    public static String _comment4 = "account_factor_comment4";
    public static String _comment5 = "account_factor_comment5";
...
//            String userId = jjTools.getSessionAttribute(request, "#" + Access_User._id.toUpperCase());
            ...
            if (userId.equals("")) {// اگر کاربر لاگین نکرده باشد
                return ("sw('ورود ثبت نام');");
            } else {//اگر کاربر لاگین کرده باشد فاکتورش را ثبت می کند
...
//                String[] formElementsKeys = new String[21];
                ArrayList<String> formElementsKeys = new ArrayList();
                Enumeration<String> parameterNames = request.getParameterNames();
                for (int j = 0; parameterNames.hasMoreElements(); j++) {
...
//                    ServerLog.Print(formElementsKeys[i] + "=" + request.getParameter(formElementsKeys[i]));
                    ...
                    ServerLog.Print(formElementsKeys.get(j) + "=" + request.getParameter(formElementsKeys.get(j)));
                }
                //<==============گرفتن پارامترها====
...
//                            ServerLog.Print("userGroup " + userGroup);
                    ...
                    ///<=========Price======
                    Integer num = i + 1;
                    if (jjTools.getParameter(request, "account_factor_pr_count_" + num).equals("")) {
                        return Js.dialog("تعداد کالای " + row.get(i).get(Product._name) + " را وارد کنید.");
                    }
                    if (Integer.parseInt(jjTools.getParameter(request, "account_factor_pr_count_" + num)) > Integer.parseInt(row.get(i).get(Product._quantity).toString())) {//اگر تعداد سفارش  بیشتر از تعداد موجود بود
//                        html.append(Js.setCss("#" + formElementsKeys[num], "borderColor", "red"));
                        ...
                        map.put("account_factor_pr_count_" + num, jjTools.getParameter(request, "account_factor_pr_count_" + num));//_pr_count_
                        map.put("account_factor_pr_fee_" + num, price);
                        map.put("account_factor_pr_sum_" + num, Integer.parseInt(price) * Integer.parseInt(jjTools.getParameter(request, "account_factor_pr_count_" + num)));
                        factorSum += Integer.parseInt(row.get(i).get(Product._price2).toString()) * Integer.parseInt(jjTools.getParameter(request, "account_factor_pr_count_" + num));
                    }
                }
//                ServerLog.Print(map);
                ...
                map.put(_comment, jjTools.getParameter(request, _comment));
                map.put(_comment2, jjTools.getParameter(request, _comment2));
                map.put(_comment3, jjTools.getParameter(request, _comment3));
                map.put(_comment4, jjTools.getParameter(request, _comment4));
                map.put(_comment5, jjTools.getParameter(request, _comment5));
                List<Map<String, Object>> factorRow = jjDatabase.separateRow(db.insert(tableName, map));

                html.append("<div id='paymentComment'></div>");// برای لود شرایط فروش
                StringBuilder script = new StringBuilder();
                script.append("new jj('do=Content.sw&text=فرم ثبت اطلاعات قبل از پیش فاکتور&panel=paymentComment&jj=1').jjAjax2(true);");
                if (factorRow.isEmpty()) {
                    ...
                    // نمایش پیش فاکتور
                    html.append("<div id='preInvoiceDiv' >");
                    html.append("<table id='preInvoceTable'>");
                    html.append("<thead>");
                    html.append("<th><div>محصول</div></th>");
                    html.append("<th><div>مقدار</div></th>");
                    html.append("<th><div>قیمت واحد</div></th>");
                    html.append("<th><div>قیمت کل</div></th>");
                    html.append("</thead>");
                    html.append("<tbody>");
                    for (int i = 1; i < 20 && !factorRow.get(0).get("account_factor_pr_id_" + i).equals(""); i++) {//برای حداکثر تعدادی که در فاکتور داریم که بیست است
                        html.append("<tr>");
                        html.append("<td><div>");
                        html.append(factorRow.get(0).get("account_factor_pr_name_" + i));
                        html.append("</div></td>");
                        html.append("<td><div>");
                        html.append(factorRow.get(0).get("account_factor_pr_count_" + i));
                        html.append("</div></td>");
                        html.append("<td><div>");
                        html.append(factorRow.get(0).get("account_factor_pr_fee_" + i));
                        html.append("</div></td>");
                        html.append("<td><div>");
                        html.append(factorRow.get(0).get("account_factor_pr_sum_" + i));
                        html.append("</div></td>");
                        html.append("</tr>");

                    }

                    html.append("<tr class='sumTr'>");
                    html.append("<td style='background-color: unset;'></td>");
                    html.append("<td style='background-color: unset;'></td>");
                    html.append("<td class='sum'><div >");
                    html.append("جمع کل");
                    html.append("</div></td>");
                    html.append("<td  class='sum'><div>");
                    html.append(factorSum);
                    html.append("</div></td>");
                    html.append("</tr>");
                    html.append("</tbody>");
                    html.append("</table>");
                    ...
                    html.append("<div id='" + _comment3 + "'>" + factorRow.get(0).get(_comment3) + "</div>");
                    html.append("<div id='" + _comment4 + "'>" + factorRow.get(0).get(_comment4) + "</div>");
                    html.append("<div id='" + _comment5 + "'>" + factorRow.get(0).get(_comment5) + "</div>");
                    html.append("</div>");

                }
                jjTools.setSessionAttribute(request, "#FACTOR_ID", factorRow.get(0).get(_id).toString());//افزودن آی دی فاکتور در سشن 
                html.append("<div class='paymentPage'>");
                html.append("<div class='siteCondition'>");
                html.append(" <input type='checkbox' id='acceptConditions' /> ");
                html.append("قبول <a href='http://localhost/002/Server?do=Content.sw&text=قوانین و شرایط' target='_blank'>قوانین وشرایط </a>");
                html.append("سایت  ");
                html.append("</div>");
                html.append("<div onclick='payment(" + factorSum + ");' class='paymentGate'>");
                html.append(" پرداخت نهایی");
                html.append("<input type='hidden' id='payment_setting_webService' name='payment_setting_webService' value='" + jjTools.getParameter(request, PaymentSetting._webService) + "' />");
                html.append("</div>");
                html.append("</div>");
                html.append("</div>");//preInvoiceDIv

                script.append(Js.setHtml("#sw", html.toString()));
