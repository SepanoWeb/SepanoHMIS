...
public class jjTime {
    ...
    public static String getDateTimeDifferent(int hour, int min, int NumberOfMin) {
        ...
        c.set(2000, 1, 1, hour, min + NumberOfMin);
//        c.add(Calendar.MINUTE, NumberOfMin);
    ...
    public static String getDateTimeDifferentSecond(int hour, int min, int second, long Second) {
        ...
        SumSec += Second;
        SumSec += second;
        int sec2 = (int) (SumSec % 60);
        int SumMin = min + (int) (SumSec / 60);
        int min3 = (int) (SumMin % 60);
        int hour2 = hour + (int) (SumMin / 60);
        String h = hour2 + "";
        String m = min3 + "";
        String s = sec2 + "";
    ...
    /**
     *
     * @param time is like "12:01:20"
     * @param Second
     * @return String is like "12:01:20"
     */
    public static String getDateTimeDifferentSecond(String time, long Second) {
        String time2[] = time.split(":");
        int hour = 0, min = 0, second = 0;
        if (time2.length == 3) {
            second = Integer.parseInt(time2[2]);
            min = Integer.parseInt(time2[1]);
            hour = Integer.parseInt(time2[0]);
            return getDateTimeDifferentSecond(hour, min, second, Second);
        }
        return "00:00:00";
    }

    public static long getDateSecondSum(int hour, int min, int second) {
        long SumSec = 0;
        SumSec += second;
        SumSec += min * 60;
        SumSec += hour * 60 * 60;
     ...
     * @return
     */
    public static long getDateSecondSum(String time) {
        long SumSec = 0;
        String time2[] = time.split(":");
        if (time2.length == 3) {
            return getDateSecondSum(Integer.parseInt(time2[0]), Integer.parseInt(time2[1]), Integer.parseInt(time2[2]));
        }
        return SumSec;
    }
    ...
    public static MyDate getDateNowPersian() {
        ...
        String s = MyCalendar.convertToPersian(d);
        Calendar c = Calendar.getInstance();
        String ss[] = s.split("/");
        MyDate m = new MyDate(
                Integer.parseInt(ss[2].trim()),
    ...
    public static MyDate getDatePersian(int dateBy_8_Length) {
        try {
            ...
            if (myDateValue.length() != 8) {
                return null;
            }
            Calendar c = Calendar.getInstance();
            java.util.Date dd = MyCalendar.convertToGregorian(
                    Integer.parseInt(myDateValue.substring(0, 4)),
                    Integer.parseInt(myDateValue.substring(4, 6)),
                    Integer.parseInt(myDateValue.substring(6, 8)) + 1);
            String s = MyCalendar.convertToPersian(dd);
            String ss[] = s.split("/");
            c.setTime(dd);
...
//            Help.dialog_ERROR_MESSAGE(ex, null, "getDatePersian()", ex.getMessage());
            return null;
        }
    }
    ...
    public static MyDate getDateGregorian(int dateBy_8_Length) {
        ...
        Calendar c = Calendar.getInstance();
        c.set(Integer.parseInt(myDateValue.substring(0, 4)),
                Integer.parseInt(myDateValue.substring(4, 6)),
                Integer.parseInt(myDateValue.substring(6, 8)));
        MyDate m = new MyDate(
                c.get(Calendar.YEAR),
    ...
    public static MyDate getDateSelectedPersian(int year, int month, int day) {
        ...
        String s = MyCalendar.convertToPersian(c.getTime());
        String ss[] = s.split("/");
        MyDate m = new MyDate(
                Integer.parseInt(ss[2]),
                Integer.parseInt(ss[1]),
    ...
    public static class MyDate {
        ...
        private static String E_Month_Name[] = {"", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December", "Junuary"};
        ...
        public int Min;
        public int Second;
        public String Year_4_length;
        public String Month_2_length;
        public String Day_2_length;
        public String Hour_2_length;
        public String Min_2_length;
        public String Second_2_length;
        public String Day_name;
        public String month_name;

        public MyDate(int Year, int Month, int Day, int Day_of_week, Calendar calendar, boolean isPersian) {
            this.Year = Year;
            this.Month = Month;
            this.Day = Day;
            ...
            this.Day_name = E_Weekdays[Day_of_week];
            this.month_name = E_Month_Name[Month];
            this.Year_4_length = jjNumber.getFormaterIntegerLength(this.Year, 4);
            this.Month_2_length = jjNumber.getFormaterIntegerLength(this.Month, 2);
            this.Day_2_length = jjNumber.getFormaterIntegerLength(this.Day, 2);
            this.Hour_2_length = jjNumber.getFormaterIntegerLength(this.Hour, 2);
            this.Min_2_length = jjNumber.getFormaterIntegerLength(this.Min, 2);
            this.Second_2_length = jjNumber.getFormaterIntegerLength(this.Second, 2);
            if (isPersian) {
                this.Day_name = P_Weekdays[Day_of_week];
                this.month_name = P_Month_Name[Month];
            }
        }
    }
